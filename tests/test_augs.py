...
def show_imgs(x, max_display=12):
    grid = make_grid(torch.from_numpy(x[:max_display]), 4).permute(
        1, 2, 0).cpu().numpy()
    plt.xticks([])
    plt.yticks([])
    plt.imshow(grid)
...
def show_stacked_imgs(x, max_display=12):
    ...
    for i in range(1, stack + 1):
        grid = make_grid(torch.from_numpy(
            x[:max_display, (i - 1) * 3:i * 3, ...]), 4).permute(1, 2, 0).cpu().numpy()
...
if __name__ == "__main__":
    ...
    tnsrs_list = list()
    folder = '../mujoco_samples/'
    sample_imgs_files = [os.path.join(folder, f) for f in listdir(
